...
mod test {
    ...
    pub fn compile_runtime(feature: Option<&str>, example_app: bool) -> PathBuf {
        ...
        let name = format!("runtime{}-emulator.bin", feature_name);
        ...
        eprintln!("{feature:?} | {feature_str:?}");
        let output =
            mcu_builder::runtime_build_with_apps(feature_str, Some(name), example_app, None, None)
