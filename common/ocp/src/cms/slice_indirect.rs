...
mod tests {
    ...
    fn imo_wrap_and_overflow_flag() {
        ...
        assert!(!region.status().overflow_cms());
        region.write(&[0x01, 0x02, 0x03, 0x04]).unwrap();
        assert!(!region.status().overflow_cms());
        assert_eq!(region.imo(), 4);
    ...
    fn read_only_region_rejects_writes() {
        ...
        assert!(region.status().read_only_error());
        assert_eq!(region.imo(), 0);
    }
    ...
    fn write_only_region_rejects_reads() {
        ...
        assert!(region.status().write_only_error());
    }
    ...
    fn status_reports_correct_metadata() {
        ...
        let s = region.status();
        assert_eq!(s.cms_region_type(), CmsRegionType::CodeSpace);
        ...
        assert_eq!(s2.cms_region_type(), CmsRegionType::Log);
