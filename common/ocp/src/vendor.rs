...
impl VendorHandler for NoopVendorHandler {
    ...
    fn hw_status(&self) -> HwStatus<'_> {
        // SAFETY: All arguments are zero/empty constants that satisfy every
        // validation check in HwStatus::new (no vendor status length overflow,
        // CompositeTemperature::NoData is always valid), so this can never fail.
